...
/*
void gaussian_fit(char *name_file, int numBins, double minX, double maxX, int
channel) {
        ...
        TH1D *h_na = getCleanHistoFromTree(name_file, numBins, minX, maxX,
channel); c[0]->cd(); h_na->Draw();
        ...
        // I fit the two peaks with a Gaussian to get better values of centroids
and sigmas

        int npoints;
        TH1F *h_fitted[0];
        float left_mk, right_mk, x_fit_min, x_fit_max, chisqr, mean, sigma;
        //int apertura_sx = 250; // arbitrary values. Per un calcolo raffinato
si può fare un secondo
        //int apertura_dx = 450; // fit dove questi valori corrispondono alle
sigma del primo fit... h_fitted[0] = (TH1F*)h_na->Clone(); // Clone on a new
TH1F to show separately the fits x_fit_min = xPeaks[0] - left_mk; // fit left
margin x_fit_max = xPeaks[0] + right_mk; // fit right margin
        // fit function
        TF1 *fit = new TF1("fit","gaus",x_fit_min,x_fit_max);
        fit->SetParameters(1,xPeaks[0]); // Fix the centroid from the already
known x fit->SetNpx(1000); // Number of points to define the function
        // fit
...
//	const = fit->GetParameter(0);
        ...
        // Results
        cout << "Fit the peak n." << 1 << ": with TSpectrum x = " << xPeaks[0]
<< endl; cout << "                    with the fit    x = " << mean << " and
sigma = " << sigma << endl; fit->Delete();
